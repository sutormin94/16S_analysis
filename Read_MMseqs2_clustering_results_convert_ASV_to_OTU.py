...
PWD="C:\\Users\sutor\OneDrive\ThinkPad_working\Sutor\Science\MetaRUS\Bioinformatics\\16S_analysis\Test_data\\"
#MMseqs2 clustering results.
MMseq_data_inpath=PWD + "seqtabnochim clu.tsv"
#Specify identity level used for clastering by MMseqs2.
MMseqs2_identity_level=98
#ASV multifasta.
ASV_fasta_inpath=PWD + "seqtabnochim.fasta"
#ASV frequences.
ASV_freq_table_inpath=PWD + "seqtabnochim.csv"
...
def analyse_clusters_dict(Clusters_dict, Identity_level, Outpath):
    ...
    plt.show()
    plt.savefig(Outpath + 'MMseqs2_clusters_size_distribution.png')    
...
def rep_seq_cumulative_frequency(ASV_input_table_path, MMseq_Clusters_dict, ASV_sequences_dict, Outpath):
    ...
    Rep_seq_freq_dataframe.to_csv(path_or_buf=Outpath+"OTU_table.csv", sep='\t', quoting=csv.QUOTE_NONNUMERIC, columns=Column_order, header=True, index=True)
